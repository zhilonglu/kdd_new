    ...
    for time_window_start in time_windows:
        ...
        for tollgate_id in volumes[time_window_start]:
            for direction in volumes[time_window_start][tollgate_id]:
                ...
                out_line=",".join([str(tollgate_id),str(time_window_start),str(direction),str(volumes[time_window_start][tollgate_id][direction])])+'\n'
                fw.writelines(out_line)
    fw.close()

def main():
    infiles=["volume(table 6)_training","volume(table 6)_training2","volume(table 6)_test2"]
    in_file = ''
